    ...
    static class MStyleMap {
        ...
        void addStyle(String key, String className) {
            try {
                ...
                this.p.put(key, Class.forName(className).newInstance());
            } catch (Exception ex) {
                ex.printStackTrace();
            }
        }
        ...
        BlockStyle get(String key) {
            return (BlockStyle) this.p.get(key);
        }

        void initMap() {
            // get all the keys
            Enumeration<?> e = this.p.propertyNames();
            ...
            while (e.hasMoreElements()) {
                ...
                Object remappedValue = this.p.get(this.p.getProperty(key));
                if (remappedValue != null) {
                    this.p.put(key, remappedValue);
                }
            }
        }
        ...
        void initStyles() {
            ...
            while (e.hasMoreElements()) {
                key = (String) e.nextElement();
                if (key.startsWith("style.")) {
                    addStyle(key, this.p.getProperty(key));
                }
            }
        }
    }
